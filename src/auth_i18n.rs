...
(function () {
  ...
  var M = {
    en: {
      ...
      forgotSmtp: 'Reset emails use configured SMTP when present, otherwise local Postfix if it is running. The message includes a time-limited link to set a new password.',
      forgotCli: 'If email cannot be delivered, a server operator can run:',
    ...
    es: {
      ...
      forgotSmtp: 'El correo de restablecimiento usa SMTP configurado si existe; si no, Postfix local cuando está en marcha. El mensaje incluye un enlace con tiempo limitado para elegir una nueva contraseña.',
      forgotCli: 'Si el correo no se puede entregar, un operador puede ejecutar:',
    ...
    nb: {
      ...
      forgotSmtp: 'E-post for tilbakestilling bruker konfigurert SMTP hvis den finnes, ellers lokal Postfix når den kjører. Meldingen inneholder en tidsbegrenset lenke for nytt passord.',
      forgotCli: 'Hvis e-post ikke kan leveres, kan en serveroperatør kjøre:',
  ...
  function apply(locale) {
    ...
    if (page === 'forgot' || page === 'forgot-ack') {
      ...
      setText('i18n-forgot-smtp', t.forgotSmtp);
      setText('i18n-forgot-cli', t.forgotCli);
      setText('i18n-forgot-back', t.forgotBack);
      return;
    }
