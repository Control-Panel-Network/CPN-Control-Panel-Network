...
download_text() {
  ...
  if have_cmd curl; then
    ...
    local -a wh=(--header="Accept: application/vnd.github+json" --header="User-Agent: CPN-upgrade.sh")
    if [[ -n "$tok" ]]; then
...
direct_release_json_for_tag() {
  ...
  local sums
  sums="$(curl -fsSL --proto '=https' --tlsv1.2 --max-time 30 -H 'User-Agent: CPN-upgrade.sh' "${base}/SHA256SUMS" 2>/dev/null)" \
    || return 1
  [[ -n "$sums" ]] || return 1
