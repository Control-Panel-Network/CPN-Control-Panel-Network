...
- **Cloudflare site DNS on create/delete**: when Cloudflare OAuth or API token is connected, website create (`/websites/create` and `cpn site create`) upserts a proxied A for the site FQDN (and `www.<fqdn>` CNAME when missing) under the matching zone. Site delete removes those CPN-managed A/AAAA/www records only (idempotent; TXT/MX/mail left alone). Zone lookup walks parent labels so subdomains like `test2.example.com` resolve the apex zone. Create/delete UI notices surface DNS success or warnings. Heal existing sites with `cpn site ready <domain>`.
